...
router.post ('/register', async (req, res) => {
  try {
    ...
    // Sign token for new registered user with JWT token
...
// (Protected Route) Get all favourites of logged in user - REACT
...
// (Protected Route) Update title of a favourite
router.post ('/favourites/:currentTitle', async (req, res) => {
  // Validate user is logged in from cookie // TODO move to external function file
  const rawCookie = req.header ('cookie').split ('=');
  let payload; // Declare variable to hold payload
  ...
  await User.findOneAndUpdate (
    {_id: payload.user},
    {$set: {'favourites.$[favourite].title': req.body.newTitle}},
    {
      arrayFilters: [{'favourite.title': req.params.currentTitle}],
      new: true,
    }
  )
    .then (doc => {
