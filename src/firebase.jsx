...
export const testGetFavourites = async () => {
...
export const testDeleteFavourite = async aTimestamp => {
  //console.log('testDeleteFavourite args: ' + aTimestamp);
  var user = firebase.auth ().currentUser; //get reference to currently logged in user
  ...
  if (snapshot.exists) {
    try {
      await userRef.update ({
        favourites: snapshot
          .data ()
          .favourites.filter (favourite => favourite.timestamp !== aTimestamp),
      });
    } catch (error) {
